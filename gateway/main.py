...
Supports streaming (SSE) and project folder routing via X-Project header.
"""
...
app = FastAPI(title="Claude Code Bridge", version="1.0.0")
...
async def chat_completions(
    ...
    # Resolve project directory for CLAUDE.md context
    ...
    if stream:
        ...
        result = await _call_claude(prompt, model, project_dir)
...
def _resolve_project(x_project: str | None) -> Path | None:
    ...
    if project_path.is_dir():
        ...
        return project_path
    log.debug("Project folder not found: %s", project_path)
    return None


async def _call_claude(prompt: str, model: str, project_dir: Path | None) -> str:
    ...
    try:
        ...
        return data.get("result", data.get("content", output))
...
async def _stream_claude(prompt: str, model: str, project_dir: Path | None):
    ...
    async for line in proc.stdout:
        ...
        # Try to parse as JSON chunk from claude
        try:
...
def _format_response(content: str, model: str) -> JSONResponse:
    """Format claude output as an OpenAI-compatible response."""
    # Approximate token counts
    prompt_tokens = 0  # We don't track input tokens
...
async def timeout_handler(request: Request, exc: TimeoutError):
    return JSONResponse(
        status_code=504,
        content={"error": {"message": str(exc), "type": "timeout_error"}},
    )
