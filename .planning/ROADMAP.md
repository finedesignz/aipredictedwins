# Roadmap â Milestone v1.0 Day-Trading Upgrade
...
(the reference `alpaca_orchestrator.py` shares the same helpers) â phases that touch "the
...
**Plans**: 1 plan
- [x] 01-01-PLAN.md â StrategyProfile module + SWING preset + parity tests + minimal orchestrator wiring
...
**Success Criteria** (what must be TRUE):
  1. A `DAYTRADE` preset configures 5-min bars, ~2-min scan, 1h HTF filter, ATR exit params, and a 4â8h max-hold.
...
**Plans**: 1 plan
- [x] 02-01-PLAN.md â DAYTRADE preset + registry, BOT_PROFILE selection at startup, banner line, tests
...
**Success Criteria** (what must be TRUE):
  1. `analyze()` reads EMA/RSI/ADX periods from the profile â no hardcoded 9/21/14 remain.
...
**Plans**: 2 plans
- [x] 03-01-PLAN.md â Parameterize analyze()/scan_assets periods, add _atr + atr_value, session-anchored VWAP, parity + new tests
- [x] 03-02-PLAN.md â Thread profile through 3 scan_assets call-sites; remediate 6 stale-threshold tests; full suite green
...
**Success Criteria** (what must be TRUE):
  1. The monitor computes an ATR-scaled stop (entry â atr_mult_stop Ã ATR) and an ATR-scaled trailing stop that ratchets up below the high-water mark.
...
**Plans**: 2 plans
- [x] 04-01-PLAN.md â Test scaffold (RED) + TrailingStop ATR-distance & short-side extension
- [x] 04-02-PLAN.md â Deterministic ATR decision ladder + hard-stop/max-hold overrides in shared PositionMonitor; thread profile into both instantiation sites
...
**Success Criteria** (what must be TRUE):
  ...
  2. Generated lessons incorporate the new intraday dimensions (e.g. "5/5 SOL, US-afternoon, low-vol â 70% WR").
  3. Until `LEARNING_SHADOW_UNTIL_TRADES` (default 30) closed trades, learning logs "WOULD veto / WOULD scale ÃN" without acting; after the threshold it auto-applies.
  4. The shadowâauto transition is controlled by a single env flag.
**Plans**: TBD
